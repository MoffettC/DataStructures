...
import data_structures.DictionaryEntry;
import data_structures.DictionaryReader;
...
public class LatinDictionary {
	...
	public String[] getRange(String start, String finish) {
		Iterator i = dictionary.keys();
		Iterator u = dictionary.keys();
		int size = 0;
		while (i.hasNext()){
			String key = (String) i.next();
			if (key.compareTo(start) >= 0 && key.compareTo(finish) <= 0){
				size++;
			}
		}
